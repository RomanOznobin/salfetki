...
class stainedglass_portfolio extends WP_Widget {
	...
	*/
	public function __construct() {
		...
		/* Widget settings. */
		$widget_ops = array(
		'classname' => 'stainedglass_portfolio',
		...
		if ( $query->have_posts() ) :
			...
			<div class="main-wrapper-image <?php echo $pos_class;?>" style="padding:<?php echo esc_attr( $instance['padding_top']);?>px <?php echo esc_attr( $instance['padding_right']);?>% <?php echo esc_attr( $instance['padding_bottom'])?>px <?php echo esc_attr( $instance['padding_left'])?>%">
				...
				<div class="description">
					<article>
						<header>
							<h3 class="main-title"><?php echo esc_html( get_term( $instance['project'], $tax )->name);?></h3>
						</header><!-- header -->
						<p><?php echo term_description( $instance['project'], $tax ) ?></p>
						<a href="<?php echo esc_url( get_term_link( $instance['project'], $tax ) ); ?>" class="link" rel="bookmark"><?php _e('All Projects', 'stainedglass'); ?></a>
					</article> <!-- article -->
				</div> <!-- .description -->

 ...
 */
add_action( 'customize_register', 'stainedglass_customize_register_fonts' );
function stainedglass_customize_register_fonts( $wp_customize ) {

	$defaults = stainedglass_get_defaults();
	
	$wp_customize->add_panel( 'fonts', array(
		'priority'       => 5,
		'title'          => __( 'Customize Fonts', 'stainedglass' ),
		'description'    => __( 'Selecting a google fonts.', 'stainedglass' ),
	) );
...
//New section in customizer: Fonts
	$wp_customize->add_section( 'fonts', array(
		'title'          => __( 'Google Fonts', 'stainedglass' ),
...
//Font for site
	$wp_customize->add_setting( 'body_font', array(
		...
		'sanitize_callback' => 'stainedglass_sanitize_font_choice'
	) );
	$wp_customize->add_control( 'body_font', array(
		'label'   => __( 'Body Font', 'stainedglass' ),
		'section' => 'fonts',
		'settings'   => 'body_font',
		'type'       => 'select',
		'priority'   => 2,
		'choices'	 => stainedglass_get_font_choices(),
	) );	
...
//Font for headings
	...
	$wp_customize->add_control( 'heading_font', array(
		'label'   => __( 'Heading Font', 'stainedglass' ),
		'section' => 'fonts',
...
//New section in customizer: Fonts Size
	$wp_customize->add_section( 'fonts_size', array(
		'title'          => __( 'Font Size', 'stainedglass' ),
		'priority'       => $section_priority++,
		'panel'  => 'fonts',
	) );
 ...
 * @since Stained Glass 1.0.0
 */
function stainedglass_sanitize_font_choice( $value ) {
	if ( '0' == $value ) {
		return '0';
	} else if ( array_key_exists( $value, stainedglass_get_font_choices() ) ) {
		return $value;
	} else {
		return 'Open Sans';
	}
 ...
 * @since Stained Glass 1.0.0
 */
function stainedglass_get_font_choices() {
	$fonts = stainedglass_get_google_fonts();
	$out = array();
...
if ( ! function_exists( 'stainedglass_get_google_fonts' ) ) :
/**
 ...
 * @since Stained Glass 1.0.0
 */
function stainedglass_get_google_fonts() {
	return array(
		'0' => array(
			'label'    => __( 'None', 'stainedglass' ), 
		),		
		'ABeeZee' => array(
			'label'    => 'ABeeZee',
			'variants' => array(

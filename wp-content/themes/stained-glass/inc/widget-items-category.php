...
class stainedglass_items_category extends WP_Widget {
	...
	*/
	public function __construct() {
		...
		/* Widget settings. */
		$widget_ops = array(
		'classname' => 'stainedglass_items_category',
		'description' => __('Display Items from the Category or latest posts.', 'stainedglass' ));

		/* Widget control settings. */
		$control_ops = array(
		'width' => 250,
		'height' => 250,
		'id_base' => 'stainedglass_items_category_widget');


		/* Create the widget. */		
		parent::__construct( 'stainedglass_items_category_widget', __( 'GL Items from Category (GLASS)', 'stainedglass' ), $widget_ops, $control_ops );
			...
			<div class="main-wrapper-image <?php echo $pos_class;?>" style="padding:<?php echo esc_attr( $instance['padding_top']);?>px <?php echo esc_attr( $instance['padding_right']);?>% <?php echo esc_attr( $instance['padding_bottom'])?>px <?php echo esc_attr( $instance['padding_left'])?>%">
				...
				<div class="description">
					<article>
						...
						<a href="<?php echo esc_url( get_term_link( $instance['category'], $tax ) ); ?>" class="link" rel="bookmark"><?php _e('Open Archive', 'stainedglass'); ?></a>
					</article> <!-- article -->
				</div> <!-- .description -->
	...
	*/
	...
	function defaults( $instance ){
		...
		$defaults = array('title' => __('Recent posts', 'stainedglass'),
						  'category'   => '0',	
						  'count'   => '4',	
						  'columns'   => 'column-4',
 ...
 * @since Stained Glass 1.0.0
 */
 function stainedglass_register_items_cat_widgets() {
	register_widget( 'stainedglass_items_category' );
}
add_action( 'widgets_init', 'stainedglass_register_items_cat_widgets' );
